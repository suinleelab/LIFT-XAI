...
STYLE = {
    "baseline_shapley_value_sampling": ("Shapley (baseline)",            PALETTE[0], "-"),
...
def aggregate_curves(arrays):
    ...
    std = np.nanstd(M, axis=0, ddof=1)/4*np.sqrt(len(M))
    return mean, std
...
def main():
    ...
    plt.ylabel(r"Distillation loss (MSE) ($\times 10^{-2}$)", size=18)
